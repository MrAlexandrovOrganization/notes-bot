...
class TestGetContext:
    def test_creates_new_context_with_idle_state(self):
    ...
    def test_creates_context_with_calendar_month_and_year(self):
        manager = StateManager()
    ...
    def test_creates_context_with_zero_task_page(self):
        manager = StateManager()
        ctx = manager.get_context(20)
        assert ctx.task_page == 0

    def test_creates_context_with_no_last_message_id(self):
        manager = StateManager()
        ctx = manager.get_context(30)
        assert ctx.last_message_id is None

    def test_different_users_have_independent_contexts(self):
        manager = StateManager()
        ctx_a = manager.get_context(100)
        ctx_b = manager.get_context(200)
        ctx_a.state = UserState.WAITING_RATING
        assert ctx_b.state == UserState.IDLE
